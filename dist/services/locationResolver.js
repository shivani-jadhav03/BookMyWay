...
export class LocationResolver {
    ...
    static async resolveTrainStations(cityName) {
        try {
    ...
    static async resolveBusStations(cityName) {
        try {
            ...
            if (response.data?.data?.documents && Array.isArray(response.data.data.documents)) {
                const stations = response.data.data.documents.map((station) => ({
                    code: station.id,
                ...
                if (cityName.toLowerCase() === 'ashta') {
                    const ashtaSangli = stations.find(station => station.name.toLowerCase().includes('sangli') &&
                    ...
                    if (ashtaSangli) {
                        ...
                        return [ashtaSangli, ...otherStations.sort((a, b) => {
                                if (a.state === 'Maharashtra' && b.state !== 'Maharashtra')
                                    return -1;
                                if (b.state === 'Maharashtra' && a.state !== 'Maharashtra')
                                    return 1;
                                return 0;
                            })];
                    }
    ...
    static async resolveAirports(cityName) {
        try {
    ...
    static async resolveAllLocations(cityName) {
